...
#Source
JPBD = FileInfo(r'm2ts/SAO_S3_Alicization_WoU_ED2.m2ts', 24, -24,
...
def main() -> vs.VideoNode:
    ...
    deband = vdf.deband.dumb3kdb(ec, threshold=48, grain=14)
    deband = core.std.MaskedMerge(deband, ec, detail_mask.std.BoxBlur(0, 4, 2, 4, 2))
    deband = lvf.misc.replace_ranges(deband, ec, [(309, 387)])
    ...
    custom_deband      = core.placebo.Deband(custom_deband, planes = 1,   grain = 18, **deband_b_args)
    custom_deband      = core.placebo.Deband(custom_deband, planes = 2|4, grain = 0,  **deband_b_args)
    custom_deband      = core.placebo.Deband(custom_deband, planes = 2|4, grain = 10,  **deband_b_args)
    deband = lvf.rfs(deband, custom_deband, (770,784))

    #Grain
    graigasm_args = dict(
        thrs=[x << 8 for x in (32, 80, 128, 176)],
        strengths=[(0.6, 0.4), (0.4, 0.3), (0.3, 0.1), (0.2, 0.1)],
        sizes=(1.25, 1.15, 1, 1),
        sharps=(80, 70, 60, 50),
        grainers=[
    ...
    extragrain = kgf.adaptive_grain(grain, 0.5, luma_scaling=2)


    #Cropping to get clean letterboxes
    crop = core.std.Crop(extragrain, 0, 0, 132, 132)
    crop = core.std.AddBorders(crop, 0, 0, 132, 132)


    return depth(crop, 10)
