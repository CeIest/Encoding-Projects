...
from vardautomation import (JAPANESE, AudioCutter,
                            ...
                            PresetBD, PresetWEB, PresetFLAC, RunnerConfig, SelfRunner,
...
#Source
WEB = FileInfo(r'm2ts/[HorribleSubs] Sword Art Online - Alicization - 14 [1080p].mkv', 3069, 5227,
                idx=lambda x: source(x),
                preset=[PresetWEB, PresetFLAC]) #CR over WAKANIM because I'm broke

JPBD = FileInfo(r'm2ts/SAO_S3_Alicization_OP2v1.m2ts', 24, -24,
                idx=lambda x: source(x),
...
def main() -> vs.VideoNode:
    ...
    src = JPBD.clip_cut
    srcweb = WEB.clip_cut
    src = src[:1421] + srcweb[1421:1544] + src[1544:] #Replacing the web scene
    src = depth(src, 32)
